...
router.post("/getAllUsers", async (req, res) => {
  try {
    ...
    let list = [-1];
    if (MyList) list = MyList.Notifications;

    let FriendArr = [-1];
    if (FList) FriendArr = FList.Friends;
    ...
    const allUsers = users.filter((ele) => {
      return (
        ele.username != uniqueId &&
        list.indexOf(ele.username) == "-1" &&
        FriendArr.indexOf(ele.username) == "-1" &&
        NotiArr.indexOf(ele.username) == "-1"
      );
    });
...
router.post("/SendNotification", async (req, res) => {
  try {
    ...
    if (Notifi) {
      ...
      const deleteUser = await NotificationRecieve.findOneAndDelete({
        userId: FriendId,
      });
    }
    ...
    if (Notifi2) {
      ...
      const deleteUser = await NotificationSent.findOneAndDelete({
        userId: userId,
      });
    }
    if (arr.indexOf(FriendId) == -1) arr.push(FriendId);
...
router.post("/getAllNotifications", async (req, res) => {
  try {
    ...
    if (MyList) return res.status(201).json({ users: MyList.Notifications });
...
router.post("/Accepted", async (req, res) => {
  try {
    ...
    const deleteUser1 = await NotificationSent.findOneAndDelete({
      userId: FriendId,
    });
    const deleteUser2 = await NotificationRecieve.findOneAndDelete({
      userId: userId,
    });
...
router.post("/saveMsg", async (req, res) => {
  try {
    ...
    if (chatData) {
      ...
      if (
        chats.length == 0 ||
        (chats.length > 0 && chats[chats.length - 1].miliTime != data.miliTime)
      ) {
    ...
    if (getChat) {
      ...
      for (let i of getChat.chats) {
        if (doc == "") {
          ...
          doc.text == i.text &&
          doc.sender == i.sender &&
          doc.reciever == i.reciever &&
          doc.miliTime == i.miliTime &&
          doc.time == i.time &&
          doc.senddate == i.senddate
        ) {
          dupli = true;
        } else {
          checkArr.push(i);
...
router.post("/getChat", async (req, res) => {
  try {
    ...
    const getChat = await Chats.findOne({ userId: sender, friendId: reciever });
    if (getChat) return res.status(201).json({ msg: getChat.chats });
...
router.post("/deleteChat", async (req, res) => {
  ...
  if (text == "Delete for me") {
    ...
    const chatData2 = await Chats.findOneAndUpdate(
      {
        userId: chat.reciever,
        friendId: chat.sender,
        chats: { $elemMatch: chat },
      },
